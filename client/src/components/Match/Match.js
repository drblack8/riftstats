...
const Match = (props) => {
	...
	useEffect(() => {
		...
		let totalKills = 0;
		for (let i = 0; i < 10; i++) {
			totalKills += props.match['participants'][i].stats.kills;
		}
		for (let i = 0; i < 10; i++) {
			if (i < 5) {
				if (
					props.match['participantIdentities'][i].player.summonerName.toLowerCase() ==
					...
					setResults({
						...
						farm: props.match['participants'][i].stats.totalMinionsKilled,
						farmpm: (
		...
		return (
			<div className={results.win === 'Win' ? 'solo-match' : 'solo-match-lost'}>
				<div className="match-info">
					...
					<div className="match-creation">{GetTime(props.match.gameCreation)}</div>
					<div className={results.win === 'Win' ? 'victory' : 'defeat'}>{winner(results.win)}</div>
				...
				<div className="player-info">
					...
					<div className="player-stats">
						...
						<div className="kda-math">
							<span className={(results.kda > 4) ? "kda-blue" : (results.kda < 1.5) ? "kda-red" : "kda-white"}>
                {(results.kda === Infinity) ? `${((results.kills + results.assists) /1).toFixed(2)} ` : `${results.kda.toFixed(2)} `}
                </span>
							<span className="kda-math-text">KDA</span>
						</div>
					</div>
					<div className="player-level">Level {results.level}</div>
				</div>
				<div className="divider"></div>
				<div className="build-info"></div>
